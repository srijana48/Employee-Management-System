...
router.post("/", function (req, res, next) {
  var empDetails = new empModel({
    ...
    total: parseInt(req.body.hrlyrate) * parseInt(req.body.ttlhr),//integer data 
  ...
  empDetails.save(function (err, res1) {
    ...
    employee.exec(function (err, data) {
      if (err) throw err;
      res.render('index', { title: 'Employee Records', records: data,success:'Record Insreted Successfully' });
...
router.post('/search/', function (req, res, next) {
  ...
  employeeFilter.exec(function (err, data) {
    if (err) throw err;
    res.render('index', { title: 'Employee Records', records: data,success:'Record Searched Successfully' });
  });
});
